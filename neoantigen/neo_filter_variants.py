...
def main(sample, in_dir, out_dir, force):
    d = sample
    #sr = int(skiprows) ## number of rows to skip for vcf
    extract = True

    if os.path.exists("{0}/{1}.neoantigens.all.vcf.tsv".format(out_dir, d)):
    ...
    if extract:
        ...
        for i in range(len(df)):
            seqs.append(df.iloc[i][7].split("|")[21])
        df["context"] = seqs

        ## extract genes
        allgenes = [] 
        for i in range(len(df)):
            allgenes.append(df[7][i].split("|")[1].split("[")[-1])
        df["GENE"] = allgenes
        col = df.pop("GENE")
        df.insert(5, "GENE", col)
        df.to_csv("{0}/{1}.neoantigens.all.tsv".format(out_dir, d), sep="\t", header=False)

        """driver_genes = ["TP53","PIK3CA","GATA3","MAP3K1","KMT2C","PTEN","CBFB","CDH1","AKT1","NF1"]
        for gene in driver_genes:
            dfgene = df[df["GENE"]==gene]
            if len(dfgene) > 0:
                dfgene.to_csv("{0}/{1}.neoantigens.{2}.tsv".format(out_dir, d, gene), sep="\t", header=False) ## ga"""
...
if __name__ == "__main__":
    ...
    main(args.sample, args.in_dir, args.out_dir, args.force)